...
namespace libtot
{
    ...
    {
#if !defined(LIBTOT_SIM_STORAGE)
        LOGE("Not in simulated storage mode!");
#endif
        auto it = sim_storage.find(block_uuid);
        if (it == sim_storage.end())
        {
            LOGW("Data block not found: %s", block_uuid.get_uuid().c_str());
            return -1;
        }

        LOGD("Reading UUID: %s", block_uuid.get_uuid().c_str());

        std::string file_path = it->second;
        return read_from_file(file_path, buf_ptr, buf_len_ptr);
    }
