...
#ifndef LIBTOT_SIEVE_HPP
#define LIBTOT_SIEVE_HPP
...
namespace libtot
{
    ...
    */
    ...
    typedef typename LIBTOT_EC_Group::Scalar RekeyToken;

    const int SIEVE_KEY_SIZE = LIBTOT_EC_Group::Scalar::SER_BYTES;
    const int SIEVE_NONCE_SIZE = 32;

    class SieveKey
    {
    private:
        LIBTOT_EC_Group::Scalar key;
        uint8_t nonce[SIEVE_NONCE_SIZE]{};

        void clone(const SieveKey &obj);

    public:
        static const int CHUNK_SIZE = LIBTOT_EC_Group::Point::HASH_BYTES;
         ...
         */
        static LIBTOT_EC_Group::Point sieve_f_k_x(LIBTOT_EC_Group::Scalar k,
                                                  unsigned int offset,
                                                  const uint8_t *nonce);
     ...
     * @return k
     */
    LIBTOT_EC_Group::Scalar sieve_keygen(
    ...
    decaf::SecureBuffer test_sieve_prf(const LIBTOT_EC_Group::Scalar &k, const char *x);

    decaf::SecureBuffer test_sieve_encrypt_block(
        const std::string &message,
        const LIBTOT_EC_Group::Point &fp);

}

#endif //LIBTOT_SIEVE_HPP
